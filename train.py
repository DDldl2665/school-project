...
class self_IAMDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #self define
        base_image_name = self.data_dict[self.indices[idx]]['base_image']
        base_img_path = os.path.join(self.image_path, base_image_name)
        base_image = Image.open(base_img_path).convert('RGB')
        base_image = self.transforms(base_image)

        
        return image, word_embedding, wr_id, base_image
class EMA:
    '''
...
# self define train
def self_train(diffusion, model, ema, ema_model, vae, optimizer, mse_loss, loader, num_classes, vocab_size, transforms, args):
    model.train()
    ...
    for epoch in range(args.epochs):
        ...
        for i, (images, word, s_id, base_image) in enumerate(pbar):
            images = images.to(args.device)
            #self define
            base_img = base_image.to(args.device)
            original_images = base_img
            text_features = word.to(args.device)
...
def main():
    ...
    parser.add_argument('--iam_path', type=str, default='./words_processedImages', help='path to iam dataset (images 64x256)')
    parser.add_argument('--gt_train', type=str, default='./gt/small_gan.iam.tr_va.gt.filter27')
    parser.add_argument('--self_gt_train', type=str, default='./gt/self_gan.iam.tr_va.gt.filter27')
    parser.add_argument('--custom_training', type=bool, default=False)
    #UNET parameters
    ...
    if args.custom_training==False:
        ...
        if args.dataset == 'iam':
            ...
            with open(args.self_gt_train, 'r') as f:
                train_data = f.readlines()
                train_data = [i.strip().split(' ') for i in train_data]
                wr_dict = {}
                full_dict = {}
                ...
                for i in train_data:
                    ...
                    base_image = i[2]
                    # print(s_id)
                    full_dict[idx] = {'image': image, 's_id': s_id, 'label':transcription, 'base_image':base_image}
                    # full_dict[idx] = {'image': image, 's_id': s_id, 'label':transcription}
                    image_wr_dict[image] = s_id
                    img_word_dict[image] = transcription
                    base_img_dict[image] = base_image
                    idx += 1
                    if s_id not in wr_dict.keys():
                        wr_dict[s_id] = wr_index
                        wr_index += 1
            ...
            train_ds = self_IAMDataset(full_dict, args.iam_path, wr_dict, args, transforms=transforms)
    ...
    if args.custom_training==False:
        ...
        self_train(diffusion, unet, ema, ema_model, vae, optimizer, mse_loss, train_loader, style_classes, vocab_size, transforms, args)
        # self_define_train(diffusion, unet, ema, ema_model, vae, optimizer, mse_loss, train_loader, style_classes, vocab_size, transforms, args)
